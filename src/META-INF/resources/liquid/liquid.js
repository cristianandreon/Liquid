//////////////////////////////////////////////////////////////////////////
//
// Liquid ver.1.23   Copyright 2020 Cristian Andreon - cristianandreon.eu
//  First update 8.1.2020 - Last update  10-08-2020
//  TODO : see trello.com
//
            ...
            if(this.bRegisterControl) {
                        ...
                        if(event.type === "rowSelected") {
                            ...
                            if(event.node.isSelected()) {
                                if(event.node.id !== liquid.lastSelectedId) {
                                    ...
                                    Liquid.refreshGrids(liquid, event.data, "selectionChange");
                            ...
                            if(iCol >= 0) {
                                ...
                                if(validateResult !== null) {
                                    if(validateResult[0] >= 0) {
                                        ...
                                        Liquid.registerFieldChange(liquid, null, event.node.data[ liquid.tableJson.primaryKeyField ? liquid.tableJson.primaryKeyField : "1" ], event.column.colId, event.oldValue, event.newValue);
                ...
                if(this.outDivObj) {
                    ...
                    if(this.mode === "lookup") {
                        ...
                        document.body.appendChild(this.lookupContainerObj2);
                    ...
                    if(this.tableJson.grids) {
                        if(this.tableJson.grids.length > 0) {
                            ...
                            if(Liquid.projectMode) {
                                ...
                                newTabHTML += "<li><a href=\"javascript:void(0)\" class=\"liquidTab liquidForeignTableEnabled\" onClick=\"Liquid.onNewLayout(event)\">" + "<img id=\"" + controlId + ".neLayout" + "\" src=\""+Liquid.getImagePath("forms2.png")+"\" style=\"width:12px; height:12px; filter:grayscale(0.7); cursor:pointer\" />" + "</a></li>";
                            }
...
var Liquid = {
    ...
    onOpenLookup:function(obj) {
        ...
        if(liquid) {
            ...
            if(obj.nodeType === 1) {
                ...
                /*
                var parentIds = "";
                var containerOverflows = ""; 
                while(parent) {
                    if( parent && parent.classList && 
                        ( parent.classList.contains('liquidLayoutRowContainer') || parent.classList.contains('liquidGridContainer') || parent.classList.contains('liquidLayoutContainer') )
                        ) {
                        var classStyle = getComputedStyle(parent);
                        parentIds += (parentIds.length ? "," : "") + parent.id;
                        containerOverflows += (containerOverflows.length ? "," : "") + classStyle.overflow;
                        parent.style.overflow = 'visible';
                    }
                    parent = parent.parentNode;
                }
                var lookupObj = liquid.lookupObj;
                lookupObj.setAttribute('container_ids', parentIds);
                lookupObj.setAttribute('container_overflows', containerOverflows);
                */
            ...
            if(liquid) {
                ...
                if(liquid.tableJson.keepOpen === true) {                
                    ...
                    if(lookupObj.nodeType === 1) {
                        /*
                        try {
                            var parentIds = lookupObj.getAttribute('container_ids').split(",");
                            var containerOverflows = lookupObj.getAttribute('container_overflows').split(",");
                            if(parentIds) {
                                for(var i=0; i<parentIds.length; i++) {
                                    document.getElementById(parentIds[i]).style.overflow = containerOverflows[i];
                                }
                            }
                        } catch(e) {}
                        */
    ...
    onForeignTable:function(obj) {
        ...
        if(liquid) {
            ...
            if(nameItems.length > 2) {
                ...
                document.getElementById(contentIdForeignTable).style.display = "";
    ...
    onNewLayoutProcess:function(obj_id, ftIndex1B) {
        ...
        if(liquid) {
            ...
            // liquid.onLoaded.push ( function() { Liquid.onContextMenu({target:{classList:["liquidOptionsLayout"], id:obj_id}}); } );
    ...
    onLayoutOptions:function(obj) {
        Liquid.onContextMenu({target:{classList:["liquidLayoutOptions"],id: typeof obj === 'string' ? obj : obj.id}});
    },
                                        ...
                                        if(detail.tables[it].table === liquid.tableJson.table || detail.tables[it].table === liquid.tableJson.schema+"."+liquid.tableJson.table) {
                                            if(detail.tables[it].ids) {
                                                ...
                                                if(!detail.tables[it].ids || !detail.tables[it].ids.length) {
                                                    if(command.name === "insert") {
                                                        // inserting record in foreign table. but no id generated : reloading...
                                                        Liquid.loadData(liquid, null, "foreignTable");
                                                    }
                                                }
                                            }
                                        }
                        ...
                        if(!Liquid.isRollbackCommand(command)) {
                            ...
                            if(isDef(liquid.tableJson.grids) && liquid.tableJson.grids.length > 0) {
                                ...
                                if(gotoGridIndex !== null) {
                                    if(gotoGridIndex>=0) {
                                        Liquid.onGridTab( document.getElementById(this.tabList[gotoGridIndex].Id) );
                                    }
                                }
                            }
                    ...
                    if(linkCount) {
                                    ...
                                    try {
                                        jQ1124(obj).datetimepicker({
                                            ...
                                            ,onShow:function(o, $input, event) {
                                                ...
                                                this.setOptions(opt);
                                                console.log(jQ1124(controlName).css('z-index'));
                                            }
                                            ,onClose:function(o) { 
                                                console.log(jQ1124(controlName).css('z-index'));
                                            }
                        ...
                        if(bSetup) {
                            if(obj) {
                                ...
                                obj.setAttribute('linkedrow1b', iRow + 1);
                                var controlName = "col." + "none" + ".row." + (iRow + 1);
                                var newId = liquid.controlId + ".layout." + layout.name;
        ...
        if(mode === "write") {
            ...
            if(itemObj.classList.contains("liquidLookup")) {
                for(var j=0; j<itemObj.parentNode.childNodes.length; j++) {
                    ...
                    if(obj.classList.contains("liquidLookupIconContainer")) {                
                        jQ1124(obj).animate( { zoom:1 }, 300, function(){ 
            ...
            if(itemObj.classList.contains("liquidLookup")) {
                for(var j=0; j<itemObj.parentNode.childNodes.length; j++) {
                    var obj = itemObj.parentNode.childNodes[j];
                    if(obj.classList.contains("liquidLookupIconContainer")) {
                        jQ1124(obj).animate( { zoom:0.6 }, 300, function(){ 
    ...
    setField:function(liquidControlOrId, field) {
    },
    ...
    createOnTabContextMenu:function( obj ) {
        ...
        if(liquid && liquid.controlId) {
            innerHTML += ""            
                ...
                +"<p class=\"liquidContextMenu-item\" onclick=\"Liquid.onNewLayout('"+liquid.controlId+".newLayout')\" >"+addImg+"<a href=\"javascript:void(0)\" >New Layout"+"</a></p>"
                ...
                + (obj.classList.contains("liquidLayoutTab") ? "<p class=\"liquidContextMenu-item\" onclick=\"Liquid.onGridOptions('"+liquid.controlId+".options','"+obj_id+"')\">"+optImg+"<a href=\"javascript:void(0)\">Grid Options"+"</a></p>" : "")
                + (obj.classList.contains("liquidLayoutTab") ? "<p class=\"liquidContextMenu-item\" onclick=\"Liquid.onLayoutOptions('"+liquid.controlId+".options','"+obj_id+"')\">"+optImg+"<a href=\"javascript:void(0)\">Layouy Options"+"</a></p>" : "")                
                +"<p><hr size=1></p>"
    ...
    createLiquidLayoutOptionsContextMenu:function( obj, layoutNameOrIndex ) {
        ...
        if(layout) {
            var onCancelCode = "Liquid.onContextMenuClose();";
            var onOkCode = "Liquid.onContextMenuClose(); Liquid.onLayoutOptionsOk('"+formId+"','"+controlId+"','"+layout.name+"');";
            var onKeyPressCode = "onkeypress=\"if(event.keyCode === 13) {"+onOkCode+"} else if(event.keyCode === 13) { "+onCancelCode+" } \"";
            ...
            contentHTML += "<table id=\""+formId+"\" cellPadding=\"3\" class=\"liquidOptionsMenu\" style=\"max-height:95%; overflow:auto; display:block;  \">"
                    ...
                    // +"<datalist id=\"sourceDataliast\"><option value=\"\"></option><option value=\"no-repeat\">/layouts/myLayout.jsp</option><option value=\"\"></option></datalist>"
                    +"</td><td>"
                    +"</td><td>"
    ...
    onContextMenu:function(e) {
        ...
        if(e) {
            ...
            || e.target.classList.contains('ag-center-cols-viewport')
                ...
                Liquid.createLiquidLayoutOptionsContextMenu(e.target);                
            } else if(e.target.classList.contains('liquidOptions')) {
                ...
                if(nameItems.length>=2) {
                    if(nameItems[1] == "grid_tab") {
                        ...
                        Liquid.createLiquidLayoutOptionsContextMenu(e.target, Number(nameItems[2])-1);
                    } else if(nameItems[1] == "document_tab") {
                    } else if(nameItems[1] == "chart_tab") {
                    }
                }
